...
class BaseTypeFuzzer():
    def __init__(self, rules=None, rule_closures=None, argname=None, contract_state_vars=[], **kwargs):
        self.rules = instantiate_rules(rules, str(
            self), self) if rules is not None else []

        possible_rules = []
        for closure in rule_closures:
            try:
                possible_rules.append(closure(self, **kwargs))
    ...
    def apply_rules(self):
        for rule in self.rules:
            ...
            if self.empty_set():
                logging.error(
                    f"\tConstraints applied to fuzzer {self} are impossible to satisfy. The rule that caused this was {rule}.")
    ...
    def __str__(self):
        ...
        raise NotImplementedError(
            f"Subclass {type(self).__name__} must override __str__")

    def validate(self, value):
        """Validates a value generated by a rule"""
        raise NotImplementedError(
            f"Subclass {type(self).__name__} must override validate")

    def next(self):
        """Generates a new random value, without any generation rules"""
        raise NotImplementedError(
            f"Subclass {type(self).__name__} must override next")
